...
use tournament::ls15_components::interfaces::{LootRequirement, Prize, PrizeToken, StatRequirement};
...
mod tournament_component {
    ...
    use tournament::ls15_components::constants::{
        ...
        VRF_COST_PER_GAME, TWO_POW_128, REGISTRATION_PERIOD
    };
    use tournament::ls15_components::interfaces::{
        ...
        AggregationMode, DataType, PragmaPricesResponse
    };
    ...
    > of ITournament<ComponentState<TContractState>> {
        ...
        fn register_tournament(
            ...
            assert(
                start_time > get_block_timestamp() + REGISTRATION_PERIOD, Errors::INVALID_START_TIME
            );
        ...
        fn submit_scores(
            ref self: ComponentState<TContractState>, tournament_id: u64, character_ids: Array<u256>
        ) {
            self._assert_tournament_is_active(tournament_id);
            let mut ls_dispatcher = ILootSurvivorDispatcher { contract_address: LOOT_SURVIVOR() };
    ...
    > of InternalTrait<TContractState> {
        ...
        fn _update_tournament_scores(
            ...
            loop {
                ...
                if score > *top_scores.at(score_index) {
            ...
            set!(self.get_contract().world(), (new_scores,));

            let new_high_score_event = NewHighScores { tournament_id, character_id, new_scores };
            self.emit(new_high_score_event.clone());
            emit!(self.get_contract().world(), (Event::NewHighScores(new_high_score_event)));
        }
