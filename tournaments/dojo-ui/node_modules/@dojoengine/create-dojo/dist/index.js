...
async function init(projectName, cwd, template) {
...
async function rewritePackageJson(projectName, clientPath) {
...
var start = new Command().name("start").description("initialize a new project with a selected template").option("-c, --cwd <cwd>", "the working directory", process.cwd()).option("-t, --template <template>", "specify the template to use").action(async (options) => {
  try {
    ...
    if (options.template) {
      ...
      if (!selectedTemplate) {
        console.error(
          `Template "${options.template}" not found. Available templates are: ${templates.map((tpl) => tpl.value).join(", ")}`
        );
        process.exit(1);
      }
...
async function main() {
  ...
  const program = new Command2().name("@dojoengine").description("install a dojo client").version(
